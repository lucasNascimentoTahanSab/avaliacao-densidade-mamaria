...
class Application:
    ...
    def open_file(self):
        ...
        self.file_name = filedialog.askopenfilename(
            title='Open an image', initialdir=os.path.normpath("Imagens/"))
        ...
        image = IMAGETEXT.PhotoImage(self.image_selected)
    ...
    def main_menu(self):
        ...
        self.options.add_command(
            label='Ler diretório de imagens de treino/teste', command=None)
        self.options.add_command(label='Treinar Classificador', command=None)
        self.options.add_command(
            label='Calcular e exibir características para imagem visualizada', command=self.descriptors)
